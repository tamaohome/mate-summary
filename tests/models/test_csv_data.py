...
def test_CSVデータの読み込み(csv_data: CSVData) -> None:
    assert isinstance(csv_data, CSVData)


def test_CSVデータの行数および列数(csv_data: CSVData) -> None:
    # CSVデータの行数
...
def test_CSVデータの内容(csv_data: CSVData) -> None:
...
def test_CSVデータをテーブル毎に取得(csv_tables: list[CSVData]) -> None:
    # テーブルの総数
    assert len(csv_tables) == 8


def test_CSVデータの行列取得(csv_tables: list[CSVData]) -> None:
    assert csv_tables[2][0] == ["#3レベル名", "材質", "形状", "寸法", "合計", "主桁", "横桁", "横構"]
    assert csv_tables[-1].cols[1] == ["材質", "SS400", "加工鋼重中計", "SS400", "購入部品中計", "合計"]


def test_CSVテーブルリストのレベル名(csv_tables: list[CSVData]) -> None:
